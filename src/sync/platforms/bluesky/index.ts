...
> = {
	...
	async create(args) {
		...
		const session = new CredentialSession(new URL(`https://${blueskyInstance}`));
		...
		async function getPostFromTid(tid?: string): Promise<ReturnType<typeof agent.getPost> | void> {
			if (!tid) {
				return;
			}
          	...
          	: undefined;
				...
				if (videos.length > 0 && videos[0].file) {
					...
					try {
						...
						logError(log, error)`Error while uploading video to bluesky: ${error}`;
					}
            ...
            twitter: Photo,
					...
					for (let i = 0; i < photos.length; i++) {
						...
						try {
							...
							logError(log, error)`Failed to parse ${photo} for bluesky: ${error}`;
						}
					}
				...
				for (let i = 0; i < post.chunks.length; i++) {
					...
					data.embed = i === 0 && firstEmbed ? firstEmbed : (await getExternalEmbedding(richText, agent));
