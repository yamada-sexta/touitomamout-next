...
export const toMetaPost = (tweet: Post): MetaPost => {
	...
	return {
		...
		async videoFiles() {
			...
			const files: DownloadedVideo[] = await Promise.all(tweet.videos.map(async (v): Promise<DownloadedVideo> => {
		...
		async photoFiles() {
			...
			const downloadedPhotos = await Promise.all(tweet.photos.map(async (photo): Promise<DownloadedPhoto> => {
