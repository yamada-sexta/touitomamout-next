...
const trimTwitterHandle = (handle: string) => handle.toLowerCase().trim().replaceAll('@', '');
...
export function envBool(key: string, defaultValue = false): boolean {
	...
	if (!res.success) {
		console.warn(`Invalid boolean for env ${key}: ${process.env[key]}, using default ${defaultValue}`);
		return defaultValue;
	}
...
export const TWITTER_USERNAME = trimTwitterHandle(process.env.TWITTER_USERNAME ?? '');
